...
def parse_args_and_config():
    ...
    parser.add_argument("--test_set", type=str, default='rice1',
...
def main():
    ...
    # 如果提供了 checkpoint 路径，则加载 checkpoint
    if args.resume:
        print("Loading checkpoint from:", args.resume)
        if hasattr(diffusion, "load_ddm_ckpt"):
            diffusion.load_ddm_ckpt(args.resume, ema=True)
        else:
            print("Warning: 当前模型不支持 load_ddm_ckpt 方法，跳过加载 checkpoint。")
